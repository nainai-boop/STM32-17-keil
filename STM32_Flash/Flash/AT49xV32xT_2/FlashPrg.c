...
/*               for Dual AT49xV32xT (32-bit Bus)                      */
/*                                                                     */
/***********************************************************************/
...
int Polling (unsigned long adr) {
  ...
  do {
    ...
    q6l = fsr.b.q6l;
    q6h = fsr.b.q6h;
    if ((fsr.b.q3l == 1) || (fsr.b.q3h == 1)) {
      break;                   // VPP not high enough
    }
...
int EraseChip (void) {
  ...
  M32(base_adr + (0x0AAA << 1)) = 0x00800080;
  M32(base_adr + (0x0AAA << 1)) = 0x00AA00AA;
  M32(base_adr + (0x1554 << 1)) = 0x00550055;
  M32(base_adr + (0x0AAA << 1)) = 0x00100010;
...
int ProgramPage (unsigned long adr, unsigned long sz, unsigned char *buf) {
  ...
  for (i = 0; i < ((sz+3)/4); i++)  {
    ...
    M32(base_adr + (0x0AAA << 1)) = 0x00AA00AA;
    M32(base_adr + (0x1554 << 1)) = 0x00550055;
    M32(base_adr + (0x0AAA << 1)) = 0x00A000A0;
