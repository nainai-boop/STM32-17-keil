...
/*               for NAND Flash programmed with Freescale i.MX27              */
/*                                                                            */
/******************************************************************************/
...
#define BOOTLOAD_ADR 0xD8000000UL       // Bootloader Address (NFC RAM)
...
static unsigned long CalcHWAddr (unsigned long adr);
...
int Init (unsigned long adr, unsigned long clk, unsigned long fnc) {
  ...
  // Initialize NAND Flash Controller to work in 8-bit mode, 
  // 4 address cycles, Small page
...
int ProgramPage (unsigned long adr, unsigned long sz, unsigned char *buf) {
  ...
  adr = CalcHWAddr (adr);
  if (ProgramPage_HW(adr, sz, buf) != NAND_OK) return (1);
  stat = ReadPage_HW(adr, sz, data_buf);
  if (stat != NAND_OK) {
...
/*- CalcHWAddr (...) -----------------------------------------------------------
 *
 *  Calculate hardware address in NAND flash for given address
 *    Parameter:     adr:       Address Requested
 *    Return Value:  hw address
 */

static unsigned long CalcHWAddr (unsigned long adr) {

  if ((adr >= BOOTLOAD_ADR) &&          // 2-nd level bootloader address
      (adr < (BOOTLOAD_ADR + block_size))) {
    adr -= base_adr;
  } else {                              // Program data address
    FindBlock(adr, 0);
    adr  = adr & (block_size-1);
    adr += nand_block_offset;
  }
  return (adr);
}
...
static int FindBlock (unsigned long adr, unsigned int restart) {
  ...
  if (restart == RESTART_SEARCH) {      // Restart block starting from beginning
    last_src_index    = -1;
    nand_block_offset =  0;
    return (0);
  }
