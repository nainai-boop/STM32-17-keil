...
int EraseSector (unsigned long adr) {
  ...
  M16(DATA_FLASH + adr - base_adr) = 0; // Dummy write to data flash
...
int ProgramPage (unsigned long adr, unsigned long sz, unsigned char *buf) {
  ...
  unsigned int i;
	...
	adr = DATA_FLASH + adr - base_adr;  // Address for access (in data flash)

	WaitFlashReady();                   // Wait flash to become ready

	for (i = 0; i < ((sz+1)/2); i++)  {
	  M16(adr) = *((unsigned short *)buf);// Write data
