...
/*               for AT91SAM7 256kB Bank 1 Flash                       */
/*                                                                     */
/***********************************************************************/
...
int Init (unsigned long adr, unsigned long clk, unsigned long fnc) {
  ...
  AT91C_BASE_MC->MC1_FMR = ((AT91C_MC_FMCN) & (clkus << 16)) | AT91C_MC_FWS_3FWS;

  // ---- BANK 1 controller
  // Set Flash Microsecond Cycle Number
  // Set Flash Waite State to max. (Single Cycle Access at Up to 30 MHz)
  AT91C_BASE_MC->MC0_FMR = ((AT91C_MC_FMCN) & (clkus << 16)) | AT91C_MC_FWS_3FWS;
  ...
  if (fnc == 2)
    switch (((AT91C_BASE_SYS->DBGU_CIDR >> 20) & 0xFF)) {
      ...
      case 0x75:                        // ARCH: for SAM7Xxx
        ...
        AT91C_BASE_MC->MC0_FCR = AT91C_MC_CORRECT_KEY | AT91C_MC_FCMD_SET_GP_NVM | (2 << 8);
        break;
    }
...
int EraseChip (void) {
  ...
  // Erase All Command
  AT91C_BASE_MC->MC1_FCR = AT91C_MC_CORRECT_KEY | AT91C_MC_FCMD_ERASE_ALL;

  // Wait until the end of Command
  while ((AT91C_BASE_MC->MC1_FSR & AT91C_MC_FRDY) != AT91C_MC_FRDY);
...
int ProgramPage (unsigned long adr, unsigned long sz, unsigned char *buf) {
  ...
  // Unlock Page Command
  AT91C_BASE_MC->MC1_FCR = AT91C_MC_CORRECT_KEY | AT91C_MC_FCMD_UNLOCK |
  ...
  AT91C_BASE_MC->MC1_FCR = AT91C_MC_CORRECT_KEY | AT91C_MC_FCMD_START_PROG |
                          (AT91C_MC_PAGEN & (page << 8));
  ...
  // Check for Errors
  if (AT91C_BASE_MC->MC1_FSR & (AT91C_MC_PROGE | AT91C_MC_LOCKE)) return (1);
